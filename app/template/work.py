...
class WorkWindow(CTkToplevel, TailorTranslate):
    ...
    def _menu_import(self, value):
        # video import
        if self.video.id == -1:
            choose_box = TLRRadioDialog(self.master,
                                        title="导入视频",
                                        text="请选择导入方式",
                                        values=[("从本地导入", 0), ("从网络导入", 1)],
                                        ok_button_text=self.translate("OK"),
                                        cancel_button_text=self.translate("Cancel"),
                                        bitmap_path=os.path.join(Paths.STATIC, self.appimages.ICON_ICO_256))
            self.dialog_show(choose_box)
            choose = choose_box.get()
            if choose == -1:
                return

            if choose == 1:
                self._import_from_online()
            else:
                self._import_from_local(value)
        else:
            message_box = TLRMessageBox(self.master,
                                        icon="warning",
                                        title=value,
                                        message=self.translate("Currently, UCut can only handle a single video. More video processing versions are coming soon."),
                                        button_text=[self.translate("OK")],
                                        bitmap_path=os.path.join(Paths.STATIC, self.appimages.ICON_ICO_256))
            self.dialog_show(message_box)

    def _import_from_local(self, value):
    ...
    def _import_from_online(self):
        download_dialog = DownloadDialog(
            master=self.master,
            bitmap_path=os.path.join(Paths.STATIC, self.appimages.ICON_ICO_256),
            tr=self.translate,
        )
        self.dialog_show(download_dialog)
    ...
    def _menu_rename(self, value):
        ...
        while True:
            input_dialog = TLRInputDialog(master=self.master,
                                          ...
                                          ok_button_text=[self.translate("OK"), self.translate("Cancel")],
                                          bitmap_path=os.path.join(Paths.STATIC, self.appimages.ICON_ICO_256),
                                          )
    ...
    def _menu_about(self, value):
        message_box = TLRMessageBox(self.master,
                                    icon="info",
                                    title=value,
                                    message=self.translate(f"UCut Version {self.app.version}.\n"),
                                    button_text=[self.translate("OK")],
                                    bitmap_path=os.path.join(Paths.STATIC, self.appimages.ICON_ICO_256))
        self.dialog_show(message_box)
